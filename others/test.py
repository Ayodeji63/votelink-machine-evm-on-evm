...
class ElectionCard(Button):
    """Enhanced election card with modern design - Now clickable!"""

    def __init__(self, title, count, status="Live", location="", screen_manager=None, **kwargs):
        ...
        self.bg_color = card_colors.get(status, (0.5, 0.5, 0.5, 1))
    ...
    def update_graphics(self, *args):
        """Update shadow and background graphics"""
        self.shadow.pos = (self.x + self.elevation, self.y - self.elevation)
        self.shadow.size = self.size
        self.bg.pos = self.pos
        self.bg.size = self.size
    ...
    def on_card_press(self, instance):
        """Handle card press - navigate to election screen"""
        print(f"Card pressed: {self.title}")  # Debug print

        if self.screen_manager:
            try:
                # Pass election data to the election screen
                election_screen = self.screen_manager.get_screen('election')
                election_screen.set_election_data(
                    title=self.title,
                    count=self.count,
                    status=self.status,
                    location=self.location
                )
                self.screen_manager.current = 'election'
                print("Navigation successful!")  # Debug print
            except Exception as e:
                print(f"Navigation error: {e}")  # Debug print
...
class ElectionScreen(Screen):
    ...
    def set_election_data(self, title, count, status, location):
        """Set election data and update UI"""
        self.election_data = {
            'title': title,
            'count': count,
            'status': status,
    ...
    def update_content(self):
        """Update content based on election data"""
        ...
        # Title
        title_label = Label(
            text=self.election_data['title'].replace('\n', ' '),
            font_size=dp(24),
            bold=True,
            color=(0.2, 0.2, 0.2, 1),
            halign='left',
        ...
        # Status
        status_label = Label(
            text=f"Status: {self.election_data['status']}",
            font_size=dp(18),
            color=(0.4, 0.4, 0.4, 1),
            halign='left',
            size_hint_y=None,
        ...
        # Location
        if self.election_data['location']:
            location_label = Label(
                text=f"Location: {self.election_data['location']}",
                font_size=dp(16),
                color=(0.4, 0.4, 0.4, 1),
                halign='left',
                size_hint_y=None,
        ...
        # Voter count
        count_label = Label(
            text=f"Counted Voters: {self.election_data['count']:,}",
            font_size=dp(18),
            color=(0.2, 0.2, 0.2, 1),
            halign='left',
            size_hint_y=None,
        ...
        placeholder_label = Label(
            text='Election Results and Details\n\nThis section can contain:\n• Live vote counts\n• Candidate information\n• Voting locations\n• Election timeline\n• Results breakdown',
            font_size=dp(16),
            color=(0.5, 0.5, 0.5, 1),
            halign='center',
            valign='middle'
        )
        placeholder_label.bind(size=placeholder_label.setter('text_size'))

        placeholder_layout.add_widget(placeholder_label)
        placeholder_card.add_widget(placeholder_layout)
    ...
    def create_races_section(self, parent):
        ...
        for election in elections:
            card = ElectionCard(
                ...
                screen_manager=self.manager  # Pass screen manager reference
            )
            cards_container.add_widget(card)

        parent.add_widget(title_label)
        parent.add_widget(instruction_label)
...
class ElectionApp(App):
    ...
    def build(self):
        ...
        election = ElectionScreen(name='election')

        sm.add_widget(dashboard)
        sm.add_widget(election)
