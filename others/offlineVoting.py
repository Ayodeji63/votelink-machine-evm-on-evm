...
# Paths
COMMIT_FILE = 'votes/commit.json'
SECRETS_FILE = 'votes/secrets.json'
...
class FingerprintVerificationPopup(Popup):
    ...
    def setup_ui(self):
        ...
        # Fingerprint icon (you can replace with actual image)
...
class QRCodeVerificationScreen(Screen):
    def __init__(self, uid, secret, vote_hash, **kwargs):
        super().__init__(**kwargs)
        self.uid = uid
        self.secret = secret
    ...
    def generate_qr_code(self):
        """Generate QR code for offline vote reveal data"""
        # Polygon Amoy testnet explorer URL
        payload = f"{self.uid}:{self.secret}"
...
def hash_vote(uid, candidate_id, secret):
    vote_data = f"{uid}:{candidate_id}:{secret}"
...
class ElectionScreen(Screen):
    ...
    def submit_vote(self, candidate_id):
        """Offline vote submission using commit-reveal scheme """
        # uid = self.verified_user.get("uid")
        # name = self.verified_user.get("name")
        uid = "23456"
        name = "Ayo Adebayo"
        ...
        secret = os.urandom(16).hex()  # Generate a random secret
        vote_hash = hash_vote(uid, candidate_id, secret)
        ...
        save_json(COMMIT_FILE, commits)

        # upload commit to IPFS
        try:
            client = ipfshttpclient.connect()
            res = client.add(COMMIT_FILE)
            cid = res["Hash"]
            print(f"Uploaded commit to IPFS with CID: {cid}")
        except Exception as e:
            self.show_error(f"Failed to upload commit to IPFS: {str(e)}")
            return

        self.switch_to_qr_screen(cid, secret)
